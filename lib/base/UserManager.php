<?php
	...
	class UserManager {
		...
		public function login($username, $password, $autologin = false, $redirect = null) {
			...
			// user does exist?
			if (!$row || !self::checkPassword($password, $row->user_password)) {
				return false;
			} else {
				...
				if (!$autologin) {
					...
					setcookie(self::COOKIE_AUTO_USERNAME, $this->user->getUsername(), self::COOKIE_AUTO_LIFETIME, '/', null, true);
					setcookie(self::COOKIE_AUTO_TOKEN, $cookieToken, self::COOKIE_AUTO_LIFETIME, '/', null, true);
